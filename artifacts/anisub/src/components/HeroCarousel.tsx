...
/**
 * Hero slide: poster asli (dari scrape OtakuDesu, instan — tidak nunggu AniList) ditaruh
 * di TENGAH, sisi kiri/kanan diisi versi blur dari poster yang sama. Ini menggantikan
 * banner AniList lama yang sering jelek/tidak ada — poster selalu ada & konsisten
 * dengan yang dipakai di kartu anime lainnya.
 */
function HeroSlide({ anime }: { anime: AnimeCard }) {
  // NOTE: anime.anilistPoster sudah di-inject server-side (dari full cache) sehingga
  // poster tersedia INSTAN tanpa perlu client-side AniList fetch. Kalau null, artinya
  // server sudah coba tapi tidak ketemu → useAnimeInfo fallback ke live resolve + MAL.
  const { poster: rawPoster } = useAnimeInfo(anime.title, anime.anilistPoster);
  const posterSrc = rawPoster ? proxyImg(rawPoster, 320) : rawPoster;
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      {/* Blurred poster fills the edges */}
      {posterSrc && (
        <img
          src={posterSrc}
          alt=""
      ...
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to bottom, rgba(5,5,18,0.55) 0%, rgba(5,5,18,0.25) 45%, rgba(5,5,18,0.65) 100%)"
      }} />
      ...
      <div className="absolute inset-0 flex items-center justify-center" style={{ paddingBottom: "56px" }}>
        <div
          className="relative"
          style={{
            width: 148, height: 210, borderRadius: 14, overflow: "hidden",
            boxShadow: "0 16px 48px rgba(0,0,0,0.7), 0 0 0 2px rgba(255,255,255,0.14)",
            flexShrink: 0,
          }}
        >
          <div className="absolute inset-0" style={{ background: grad }} />
          {posterSrc && (
            <img
              ...
              alt={anime.title}
              className="absolute inset-0 w-full h-full object-cover"
              style={{ opacity: posterLoaded ? 1 : 0, transition: "opacity 0.5s ease" }}
...
export default function HeroCarousel({ items, totalCount }: { items: AnimeCard[]; totalCount?: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Gradient overlays — hanya untuk keterbacaan teks di bawah, banner tetap terang */}
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to top, rgba(5,5,18,0.95) 0%, rgba(5,5,18,0.25) 30%, transparent 55%)"
      }} />
      <div className="absolute inset-0" style={{
        background: "linear-gradient(to right, rgba(5,5,18,0.35) 0%, transparent 45%)"
      }} />
