...
import { fetchSchedule, fetchAllCompletedIds, fetchAllOngoingMap } from "@/lib/api";
import { usePoster } from "@/lib/usePoster";
import { titlePlaceholder, proxyImg } from "@/lib/utils";
import { useAniListFinishedTitles, useAniListReleasingTitles, isCompletedHeuristic, titleKey } from "@/lib/completion";
...
function SchedulePoster({ title, accent }: { title: string; accent: string }) {
  ...
  return (
    ...
    >
      ...
      {malPoster && !err && (
        <img
          src={proxyImg(malPoster, 120)} alt={title}
...
export default function SchedulePage() {
  ...
  // Anime yang sudah tamat (TAMAT/END) — dipakai untuk auto-remove dari jadwal.
  // Ambil semua halaman completed (bukan cuma page 1) biar lebih akurat & instan.
  const { data: completedIdsData } = useQuery({
    queryKey: ["completed-ids-all"],
    queryFn: () => fetchAllCompletedIds(),
    staleTime: 30 * 60_000,
  });
  // instanceof guard: cache lama/corrupt yang bukan Set asli tidak boleh bikin app crash.
  const completedIds = completedIdsData instanceof Set ? completedIdsData : new Set<string>();

  // Map lengkap semua anime ongoing (status + tanggal rilis terakhir) — dipakai untuk
  // memperkaya data schedule (yang cuma punya title+animeId) sebelum heuristik 7 hari dicek.
  ...
  const dayCandidates = todaySchedule?.animeList ?? [];

  // ── Semua judul unik dari semua hari (untuk AniList override seluruh minggu) ──
  const allWeekTitles = useMemo(() => {
    const seen = new Set<string>();
    const titles: string[] = [];
    for (const day of scheduleList) {
      if (!DAYS.includes(day.title)) continue;
  ...
  // AniList override untuk semua judul minggu ini — sama persis seperti Home carouselList
  const allWeekFinished = useAniListFinishedTitles(allWeekTitles);
  const allWeekReleasing = useAniListReleasingTitles(allWeekTitles);

  // ── Total unik semua hari (minggu ini), filter identik dengan Home carouselList ──
  const totalWeeklyOngoing = useMemo(() => {
    if (!scheduleList.length) return 0;
    const seen = new Set<string>();
    let count = 0;
    for (const day of scheduleList) {
      if (!DAYS.includes(day.title)) continue;
      for (const a of day.animeList) {
        ...
        if (allWeekReleasing.has(tk)) { count++; continue; }  // RELEASING override positif
        if (allWeekFinished.has(tk)) continue;                 // FINISHED = tamat
    ...
    return count;
  }, [scheduleList, completedIds, ongoingMap, allWeekFinished, allWeekReleasing]);

  // Cek status AniList untuk anime hari ini — sumber paling akurat
  const dayTitles = dayCandidates.map((a) => a.title);
  const aniListFinishedTitles = useAniListFinishedTitles(dayTitles);
  // RELEASING override: anime yang hiatus lalu lanjut tayang — AniList RELEASING
  // harus override exclusion dari completedIds agar muncul kembali di jadwal
  const aniListReleasingTitles = useAniListReleasingTitles(dayTitles);

  // Urutan prioritas filter (sama dengan Home carousel, biar konsisten):
  // 1. AniList RELEASING     → pasti ongoing, override segalanya (kasus hiatus berakhir)
  // 2. AniList FINISHED      → pasti tamat, keluarkan
  // 3. OtakuDesu completedIds → tamat menurut sumber, keluarkan
  // 4. Heuristik 7 hari      → nggak ada eps baru 7+ hari = auto tamat, keluarkan dari jadwal
  const rawAnimeList = dayCandidates.filter((a) => {
    const tk = titleKey(a.title);
    if (aniListReleasingTitles.has(tk)) return true;   // override positif
    if (aniListFinishedTitles.has(tk)) return false;
    if (completedIds.has(a.animeId)) return false;
    const enriched = { ...ongoingMap.get(a.animeId), ...a };
    if (isCompletedHeuristic(enriched)) return false;
    return true;
  });
