...
function PlayerCore({
  ...
  // Show controls; auto-hide after 4s when playing.
  // Works for ALL modes (direct video + iframe): hide when playing, stay when paused.
  ...
  // When entering landscape mode: force controls visible then auto-hide after 4s if playing.
  useEffect(() => {
    if (controlsTimerRef.current) clearTimeout(controlsTimerRef.current);
    setShowControls(true);
    if (isLandscape && isPlayingRef.current) {
      controlsTimerRef.current = setTimeout(() => setShowControls(false), 4000);
    }
  }, [isLandscape]);

  // ── Ping measurement — ukur latensi ke server tiap 1.5 detik ─────────────────
  useEffect(() => {
    const measure = async () => {
      try {
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (e.data.type === 'ANISUB_TIME') {
        ...
        if (playing) { showControlsTemporarily(); } else { if (controlsTimerRef.current) { clearTimeout(controlsTimerRef.current); controlsTimerRef.current = null; } setShowControls(true); }
      }
    };
  ...
  const handlePlayPause = () => {
    if (isDirectVideo) {
      ...
      if (v) {
        ...
        else { v.pause(); setIsPlaying(false); isPlayingRef.current = false; onPlayStateChange?.(false); if (controlsTimerRef.current) { clearTimeout(controlsTimerRef.current); controlsTimerRef.current = null; } setShowControls(true); }
      } else {
        ...
        if (nowPlaying) { showControlsTemporarily(); } else { if (controlsTimerRef.current) { clearTimeout(controlsTimerRef.current); controlsTimerRef.current = null; } setShowControls(true); }
      }
    }
  };
  ...
  const onGestureTouchEnd = (e: React.TouchEvent) => {
    ...
    const now = Date.now();
    const container = containerDivRef.current;
    // Double-tap to seek (direct video only in landscape)
    if (isLandscape && isDirectVideo && now - lastTapTimeRef.current < 350) {
      const isLeft = container ? ct.clientX < container.clientWidth / 2 : false;
      const seekSec = isLeft ? -10 : 10;
      if (onSeek && durationSec > 0) {
        onSeek(Math.max(0, Math.min(1, (elapsedSec + seekSec) / durationSec)));
      }
  ...
  const videoContent = isDirectVideo ? (
    <video
      ...
      onPause={() => { isPlayingRef.current = false; setIsPlaying(false); onPlayStateChange?.(false); if (controlsTimerRef.current) { clearTimeout(controlsTimerRef.current); controlsTimerRef.current = null; } setShowControls(true); }}
