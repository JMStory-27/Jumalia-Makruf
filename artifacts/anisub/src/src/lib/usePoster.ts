...
export function useAnimeInfo(title: string, serverPoster?: string | null): AnimeInfo {
  const [info, setInfo] = useState<AnimeInfo>(() => {
    if (!title) return { poster: null, currentEp: null, nextAiringAt: null };
    // Server sudah resolve poster — pakai langsung
    if (serverPoster) return { poster: serverPoster, currentEp: null, nextAiringAt: null };
  ...
  useEffect(() => {
    ...
    if (serverPoster) {
      ...
      const cached = getCachedBanner(title);
      if (!cached) {
        // Fetch metadata (currentEp, nextAiringAt) di background tanpa blokir poster
    ...
    if (serverPoster === null) {
      ...
      const cached = getCachedBanner(title);
      if (cached?.cover) {
        setInfo({ poster: cached.cover, currentEp: cached.currentEp ?? null, nextAiringAt: cached.nextAiringAt ?? null });
        return;
      }

      // Cek _resolveCache in-memory
    ...
    fetchAniListBannerOnDemand(title)
      .then((entry) => {
        if (cancelled) return;
        const { cover, currentEp, nextAiringAt } = entry;
        if (cover) {
          setInfo({ poster: cover, currentEp: currentEp ?? null, nextAiringAt: nextAiringAt ?? null });
        } else {
          setInfo((prev) => ({ ...prev, currentEp: currentEp ?? null, nextAiringAt: nextAiringAt ?? null }));
