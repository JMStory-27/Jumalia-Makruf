...
const FIXMERAH_DIR   = path.join(WORKSPACE, 'artifacts', 'fix-merah');
const DIST_DIR       = path.join(FIXMERAH_DIR, 'dist', 'ghpages');

const GH_OWNER           = process.env.GITHUB_OWNER || 'JMStory-27';
const GH_REPO            = process.env.GITHUB_REPO  || 'Jumalia-Makruf';
const GH_PAGES_FIXMERAH  = `https://${GH_OWNER.toLowerCase()}.github.io/Jumalia-Makruf/FixMerah/`;
...
/* ── Scan perubahan fix-merah vs git HEAD ───────────────────────────────── */
function scanChanges() {
  const changes = { modified: [], added: [], deleted: [], recent: [] };
  try {
    const diff = execSync(
      'git diff --name-status HEAD -- artifacts/fix-merah/ artifacts/api-server/src/routes/fixmerah.ts',
    ...
    const log = execSync(
      'git log --oneline -5 -- artifacts/fix-merah/ artifacts/api-server/src/routes/fixmerah.ts',
      { cwd: WORKSPACE, encoding: 'utf8', timeout: 10000 }
    ).trim();

    if (log) changes.recent = log.split('\n').slice(0, 5);

    const status = execSync(
      'git status --short -- artifacts/fix-merah/ artifacts/api-server/src/routes/fixmerah.ts',
      { cwd: WORKSPACE, encoding: 'utf8', timeout: 10000 }
    ).trim();
...
/* ── Build Fix Merah untuk GH Pages ─────────────────────────────────────── */
async function buildFixMerah(onProgress) {
  const log = (m) => { if (onProgress) onProgress(m); };
  log('🔨 Build Fix Merah untuk GitHub Pages...');
  ...
  try {
    execSync('pnpm --filter @workspace/fix-merah run build:ghpages', {
      env,
      cwd:    WORKSPACE,
      stdio:  'pipe',
      timeout: 5 * 60 * 1000,
...
/* ── Deploy ke gh-pages branch (subfolder FixMerah/) ──────────────────── */
async function deployFixMerah(onProgress) {
  const log   = (m) => { if (onProgress) onProgress(m); };
  const token  = process.env.GITHUB_TOKEN;
  ...
  for (const { full, rel } of walkDir(DIST_DIR)) {
    files.push({ remotePath: `FixMerah/${rel}`, buffer: fs.readFileSync(full) });
  }
  ...
  const commitPayload = {
    message: `deploy: update Fix Merah — ${now}`,
    tree:    newTree.sha,
    author:  { name: 'FixMerahBot', email: 'bot@fixmerah.id', date: now },
  };
...
module.exports = { buildFixMerah, deployFixMerah, scanChanges, GH_PAGES_FIXMERAH, DIST_DIR };
