...
const { buildFixMerah, deployFixMerah, scanChanges, GH_PAGES_FIXMERAH } = require('./fixmerahBuild');
...
function getReplitUrl() {
  ...
  if (dev) return `https://${dev}/fixmerah/`;
  const domains = process.env.REPLIT_DOMAINS;
  if (domains) return `https://${domains.split(',')[0].trim()}/fixmerah/`;
  return null;
}
...
async function takeScreenshot(url, waitMs = 8000) {
  try {
    ...
    console.error('[pushfixmerah] screenshot API:', JSON.stringify(data).slice(0, 200));
  } catch (e) {
    console.error('[pushfixmerah] screenshot error:', e.message);
  }
  return null;
}
...
/* ── Main push flow ──────────────────────────────────────────────────────── */
async function runPushFixMerah(bot, chatId) {
  ...
  const scanMsg = await bot.sendMessage(
    chatId,
    `🔴 *\\[Fix Merah\\] Memindai perubahan\\.\\.\\.*\n\n` +
    `📂 Membaca source code fix\\-merah\\.\\.\\.\n` +
    `⏳ Sebentar\\.\\.\\.`,
  ...
  let scanText = `🔍 *\\[Fix Merah\\] Hasil Scan Source Code*\n\n`;
  ...
  scanText +=
    `🎯 *Target deploy:*\n` +
    `\`${esc(GH_PAGES_FIXMERAH)}\`\n\n` +
    `👇 *Klik tombol di bawah untuk konfirmasi push:*`;

  const confirmKey = `pfm:${chatId}:${scanMsg.message_id}`;
  ...
  /* ── Step 2: Tunggu konfirmasi (timeout 5 menit) ── */
  ...
  const edit = async (pct, step) => {
    try {
      await bot.editMessageText(
        `🚀 *Push Fix Merah ke GitHub Pages\\.\\.\\.*\n\n` +
        `\\[${esc(bar(pct))}\\] *${pct}%*\n` +
        `🔧 ${esc(step)}\n` +
  ...
  try {
    await edit(5, 'Build Fix Merah (compile TypeScript + bundle assets)...');
    await buildFixMerah((step) => edit(15, step));
    ...
    const deployResult = await deployFixMerah((step) => edit(50, step));
    ...
    if (ghPagesBuf) {
      await bot.sendPhoto(chatId, ghPagesBuf, {
        ...
        parse_mode: 'MarkdownV2',
        filename:   'github-pages-fixmerah.png',
      });
    } else {
      await bot.sendMessage(chatId,
        `🌐 *GitHub Pages*\n🔗 \`${esc(GH_PAGES_FIXMERAH)}\`\n\n` +
    ...
    await bot.sendMessage(chatId,
      `📊 *Hasil Verifikasi Push Fix Merah*\n\n` +
...
function registerPushFixMerahCommand(bot) {
  bot.onText(/^\/pushfixmerah(?:\s|$)/i, async (msg) => {
    await runPushFixMerah(bot, msg.chat.id);
  });
  ...
  bot.on('callback_query', async (query) => {
    ...
    try {
      await bot.answerCallbackQuery(query.id, {
        text: isOk ? '✅ Push Fix Merah dikonfirmasi! Proses dimulai...' : '❌ Push dibatalkan.',
        show_alert: false,
      });
    } catch (_) {}
  });
...
module.exports = { registerPushFixMerahCommand };
